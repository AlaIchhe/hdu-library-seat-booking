...
def is_time_out_of_range(result: dict) -> bool:
    """判断预约结果是否为"超出时间范围"错误。"""
    return MSG_TIME_OUT_OF_RANGE in _get_message(result)


def is_duplicate(result: dict) -> bool:
    """判断预约结果是否为"重复预约"错误。"""
    return MSG_DUPLICATE in _get_message(result)


def is_seat_unavailable(result: dict) -> bool:
    """判断预约结果是否为"座位不可用"错误。"""
    return MSG_SEAT_UNAVAILABLE in _get_message(result)


def is_invalid_request(result: dict) -> bool:
    """判断预约结果是否为"非法请求"错误。"""
